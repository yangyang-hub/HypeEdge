...
| 资金费套利 | **主线（单所内对冲）** | 单腿收 funding 是裸方向暴露，已弃；改用 HL 永续空 + HL 现货多对冲，单所内即 delta-neutral，无需跨所基建（§7.0） |
...
- 与「资金费套利」共享跨所基建诉求；但资金费套利的单所内对冲形态（HL 空永续 + HL 现货多）已在 §7.0 独立实现，不依赖本节跨所基建。本节跨所基差套利仍为后期项。
- 单所阶段不启动。
...
- [ ] 跨所基差套利的第二交易所选型（后期）。
- [ ] 资金费套利：补全 HL 现货行情/下单与两腿对冲执行（控制面已就绪，见 §7.0）。
